...
volatile int benchtimerexpired=0;
...
void help(){
    printf("mywebbench url benchtime client\n");
    printf("benchtime: defult 10s\n");
...
{   
    ...
    for(int i=0;i<client;i++){
        int e = pthread_create(&pd[i],NULL,bench_pth,NULL);
        if(e != 0){
            errpintf("pthread_create\n");
        }
        // e = pthread_detach(pd[i]);
        // if(e != 0){
        //     errpintf("pthread_detach\n");
        // }
    }
    sleep(benchtime);
...
void benchcore()
{
    ...
    // alarm(benchtime); // after benchbenchtime,then exit

    rlen=strlen(request);
    nexttry:while(1)
    {
        if(benchtimerexpired)
        {
            if(failed>0)
            {
